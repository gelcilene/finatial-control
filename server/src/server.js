...
async function serializeAccounts(rows) {
...
app.get("/accounts", async (_req, res) => {
  try {
    ...
    const accounts = await serializeAccounts(result.rows);
    res.json(accounts);
  } catch (err) {
    console.error(err);
...
app.post("/accounts", async (req, res) => {
  try {
    ...
    const accounts = await serializeAccounts(insert.rows);
...
app.post("/accounts/:id/payments", async (req, res) => {
  try {
    ...
    if (!password || !proofName || !proofBase64) {
      return res
        .status(400)
        .json({ error: "Campos obrigatórios: password, proofName, proofBase64" });
    }
    ...
    const accounts = await serializeAccounts(refreshed.rows);
    res.status(201).json(accounts[0]);
  } catch (err) {
    console.error(err);
...
async function bootstrap() {
  await ensureSchema();
  app.listen(PORT, () => {
    console.log(`API Finatial Control ouvindo na porta ${PORT}`);
  });
}
